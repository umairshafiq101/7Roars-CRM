...
import { getWorkTimesData } from "@/actions/work-times";
import { SummaryCards } from "@/components/modules/work-times/SummaryCards";
import { TeamGroup } from "@/components/modules/work-times/TeamGroup";
import {
  ChevronLeft,
  ChevronRight,
  Calendar,
...
import type { WorkTimesData, WorkTimesEmployee } from "@/actions/work-times";
...
export default function WorkTimesPage() {
  ...
  const [data, setData] = useState<WorkTimesData | null>(null);
  const [loading, setLoading] = useState(true);
  const [allEmployees, setAllEmployees] = useState<WorkTimesEmployee[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await getWorkTimesData({
      ...
      if (result.success && result.data) {
        const d = result.data as WorkTimesData;
        setData(d);
        // Keep full employee list for filter dropdown (only on first load)
        if (allEmployees.length === 0) {
          setAllEmployees(d.employees);
        }
      }
    } catch (err) {
      console.error("Failed to fetch work times:", err);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (allEmployees.length === 0) {
      ...
      getWorkTimesData({ startDate: start.toISOString(), endDate: end.toISOString() }).then((res) => {
        if (res.success && res.data) {
          setAllEmployees((res.data as WorkTimesData).employees);
        }
      });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const grouped: Record<string, WorkTimesEmployee[]> = {};
  ...
  function exportCSV() {
    if (!data) return;
    const header = "Employee,Role,Activity %,Working,Break,Idle,First Clock-in,Last Clock-out,Late Clock-in";
    const lines = data.employees.map((e) =>
      `"${e.name}","${e.role}","${e.activityLevel}%","${fmtHM(e.workingSeconds)}","${fmtHM(e.breakSeconds)}","${fmtHM(e.idleSeconds)}","${fmtTime(e.firstClockIn)}","${fmtTime(e.lastClockOut)}","${fmtHM(e.lateSeconds)}"`
    );
    ...
    a.href = url;
    a.download = `work-times-${startDate.toISOString().split("T")[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }

  const totalHM = fmtHM(data?.summary.totalTrackedSeconds || 0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Work times</h1>
          <p className="text-sm text-[var(--muted-foreground)]">
            You can reach your organization work times insights here.
          </p>
        </div>
      </div>
          ...
          {(!data || data.employees.length === 0) && (
            <div className="flex flex-col items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--background)] py-16 text-center">
              ...
              <p className="mt-1 text-xs text-[var(--muted-foreground)]">
                No work time data found for the selected period.
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
              {ROLE_ORDER.map((role) =>
                grouped[role]?.length > 0 ? (
                  <TeamGroup key={role} role={role} employees={grouped[role]} />
                ) : null
              )}
            </div>
          )}
